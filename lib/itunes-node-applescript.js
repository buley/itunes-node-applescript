...
var scripts = {
    ...
    playPlaylistArtist:
        ['tell application "iTunes" to play (playlist "%s")', 'tell application "iTunes" to play (first track of playlist "%s" whose artist is "%s")'],
    playArtist:
...
var execScript = function(scriptName, params, callback){
    ...
    script = scripts[scriptName];
		...
		res.push(applescript.execFile(scriptsPath + script.file, callback));
		    ...
		    res.push(applescript.execString(script.join("\r\n"), callback));
	     }
	if (1 === res.length) {
		return res[0];
	}
...
exports.playPlaylistArtist = function(playPlaylist, playArtist, callback){
	console.log('itunes.playPlaylistArtist',playPlaylist, playArtist, callback);

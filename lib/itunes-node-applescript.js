...
var scripts = {
    ...
    track:
	'tell application "iTunes" to get {id, artist, name, album, genre, index} of the current track',
    volumeUp:
...
var execScript = function(scriptName, params, callback){
    ...
    clone = scripts[scriptName];
		...
		console.log("FILE: " + clone.file, typeof callback);
...
exports.artwork = function(callback){
    return execScript('artwork', function(src) {
        src = src || '';
	console.log("SCRIPT EXEC",src);
	callback(src.replace('file ":Macintosh HD', ''))
    });
};

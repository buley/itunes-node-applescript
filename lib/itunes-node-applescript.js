...
var scripts = {
    ...
    commandHotkey:
	['tell application "OBS" to activate', 'tell application "System Events" keystroke "%s" using command down'],
    shiftHotkey:
	['tell application "OBS" to activate', 'tell application "System Events" keystroke "%s" using shift down'],
    commandShiftHotkey:
	['tell application "OBS" to activate', 'tell application "System Events" keystroke "%s" using (shift down, command down)']
...
var execScript = function(scriptName, params, callback){
    ...
    script = scripts[scriptName];
		...
		res.push(applescript.execString(script, callback));
	    } else if (!!script && script.file){
		res.push(applescript.execFile(scriptsPath + script.file, callback));
	    } else if ('string' !== typeof script && script.length > 0) {
                    e = script;
		    xlen = script.length;
		    for(x = 0; x < xlen; x += 1) {
			if (null !== script[x].match(/%/)) {
				if (typeof params !== 'undefined') script[x] = util.format.apply(util, [script[x]].concat(params));
			}
			console.log("SCRIPT: " + script[x]);
			res.push(applescript.execString(script[x], callback));
		    }
	     }
	if (1 === res.length) {
		return res[0];
